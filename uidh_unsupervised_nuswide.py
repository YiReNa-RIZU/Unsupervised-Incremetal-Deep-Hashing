...
def train(
    ...
    """
    print('start dataset2 unsupervised step')
    print('down mode is: unsupervised')
    ...
    pre_model.classifier = pre_model.classifier[:-3]
    ...
    for it in range(max_iter):
        ...
        with torch.no_grad():
            train_dataloader, sample_index = sample_dataloader(retrieval_dataloader, num_samples, batch_size, root2, dataset2)
            S = make_similarity_matrix(train_dataloader, retrieval_dataloader, pre_model)
        ...
        iter_loss = calc_loss(U, U_old_save, old_F_data, B, S, code_length, sample_index, gamma, loss_type)
...
def calc_loss(U, U_old_save, old_F_data, B, S, code_length, omega, gamma, loss_type):
    """
    Calculate loss.
    """
    ...
    hash_loss = ((code_length * S - U @ B.t()) ** 2).sum()
    quantization_loss = ((U - B[omega, :]) ** 2).sum()
    #S_old = torch.ones(U_old_save.shape[0],old_F_data.shape[0]).cuda()
    replace_loss = ((code_length - (U_old_save * old_F_data).sum(1)) ** 2).sum()
    if loss_type == 'uidh':
        loss = (hash_loss + gamma * quantization_loss) / (U.shape[0] * B.shape[0]) + 0.05 * replace_loss / (U_old_save.shape[0])
    elif loss_type == 'adsh':
        loss = (hash_loss + gamma * quantization_loss) / (U.shape[0] * B.shape[0])
    else:
...
def generate_similarity_matrix(cos_dist, alpha=1, beta=3):
    """
    ...
    return torch.FloatTensor(S).cuda()


def make_similarity_matrix(train_dataloader,retrieval_dataloader,model):
    model.cuda()
    model.eval()
    start_time = time.time()
    with torch.no_grad():
        S1 = torch.zeros(len(train_dataloader.dataset),4096).float().cuda()
        S2 = torch.zeros(len(retrieval_dataloader.dataset),4096).float().cuda()
        SM = torch.zeros(len(train_dataloader.dataset),len(retrieval_dataloader.dataset)).float().cuda()
        ...
        for i,(image,label,index) in enumerate(retrieval_dataloader):
            image = image.cuda()
            output = model(image)
            S2[index,:] = output
        ...
        for i in range(len(S1)):
            cos_dis = torch.cosine_similarity(S1[i].unsqueeze(0),S2,dim=1)
            SM[i,:] = cos_dis
        
        SM = 1-SM
        SM = generate_similarity_matrix(SM.cpu().numpy())
        
        print('[time]:',time.time()-start_time)
        print('finish make similarity matrix')
