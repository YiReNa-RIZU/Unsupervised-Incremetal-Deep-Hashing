...
def train(
    ...
    similarity_matirx = torch.load(os.path.join('checkpoints','similarity_maxtrix.t')).to(device).float()
    ...
    for it in range(max_iter):
        ...
        with torch.no_grad():
            ...
            train_targets = similarity_matirx[sample_index]
            retrieval_targets = similarity_matirx
            S = (train_targets @ retrieval_targets.t() > 0).float()
            
            #S = (train_dataloader.dataset.get_onehot_targets().to(device) @ retrieval_dataloader.dataset.get_onehot_targets().to(device).t() > 0).float()
