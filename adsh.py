...
def train(
    ...
    """
    print('start dataset1 supervised step')
    # Initialization
    ...
    for it in range(max_iter):
        ...
        if mAP > best_mAP:
            ...
            # Save checkpoints
            torch.save(query_code.cpu(), os.path.join('checkpoints', 'query_code1_up.t'))
            torch.save(B.cpu(), os.path.join('checkpoints', 'B_dataset1_up.t'))
            ...
            torch.save(model.cpu(), os.path.join('checkpoints', 'model_supervised.t'))
            torch.save(best_mAP, os.path.join('checkpoints', 'best_map_up_data1.t'))
            model.cuda()
            model.train()
    ...
    model.train()
    loss_plot(hist_loss,'./','data1_up_1')
    
    
    
    
    #construct dataset2 up code
    model = torch.load(os.path.join('checkpoints', 'model_supervised.t'))
    best_mAP = torch.load(os.path.join('checkpoints', 'best_map_up_data1.t')) 
    model.cuda()
    
    print('load model best_map is %.4f'%best_mAP)
    retrieval_code2 = generate_code(model, retrieval_dataloader2, code_length, device)
    torch.save(retrieval_code2.cpu(), os.path.join('checkpoints', 'U_database2_up.t'))
    model.train()
    print('finish construct dataset2 up_code,size is %d x %d'%(retrieval_code2.size(0), retrieval_code2.size(1)))
    
    
    query_code2 = generate_code(model, query_dataloader2, code_length, device)
    torch.save(query_code2.cpu(), os.path.join('checkpoints', 'query_code2_up.t'))
    map_dataset2 = evaluate.mean_average_precision(
        query_code2.to(device),
        retrieval_code2.to(device),
        query_dataloader2.dataset.get_onehot_targets().to(device),
        retrieval_dataloader2.dataset.get_onehot_targets().to(device),
        device,
        topk2,
    )
    model.train()
    print('the map of dataset2 up_code is %.4f'%map_dataset2)
